...
class Scanner(in: Reader[Char]) extends Reader[Tokens.Token] with Reporter {
  ...
  val isKeyword    = Set[String]("if","else","while","var","val")
  ...
  def getNum() = {
    ...
    val maxint = (1 << 32)-1
    if (res < 0 && res > maxint) abort(s"overflow")
    else Number(res)
  }
...
class ArithParser(in: Scanner) extends Parser(in) {
  ...
  def parseExpression(min: Int): Exp = {
    var check = 1
    var res = parseUAtom
    // TODO: complete use isInfix
    while (in.hasNext(isOperator) && check == 1) {
      val Ident(x) = in.peek
      if (isInfixOp(min)(Ident(x))) {
        var (op, pos) = getOperator
        res = Prim(op, res, parseExpression(prec(op)+check).withPos(pos))
      } else {
        check = 0
      }
    }
...
class LetParser(in: Scanner) extends ArithParser(in) {
  ...
  override def parseAtom: Exp = in.peek match {
    ...
    case Number(x) =>
      ...
      Lit(x).withPos(pos)
    case Ident(x) =>
      // TODO: remove ??? and add the implementation for that case(have problem)
      val (_, pos) = getName
      Ref(x).withPos(pos)
...
class BranchParser(in: Scanner) extends LetParser(in) {
  ...
  override def parseSimpleExpression = in.peek match {
    case Keyword("if") =>
      accept("if")
      accept('(')
      val pc = parseCondition
      accept(')')
      val tbranch = super.parseSimpleExpression
      accept("else")
      val ebranch = super.parseSimpleExpression
      If(pc,tbranch,ebranch)
...
class VariableParser(in: Scanner) extends BranchParser(in) {
  ...
  override def parseExpression = in.peek match {
    case Keyword("var") =>
      ...
      VarDec(name,rhs,res).withPos(pos)
    case _ => parseSimpleExpression
  }
  ...
  override def parseSimpleExpression = (in.peek, in.peek1) match {
    case (Ident(x), Delim('=')) =>
      val (_, pos) = getName
      accept('=')
      val res = super.parseSimpleExpression
