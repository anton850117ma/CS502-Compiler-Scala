...
class Scanner(in: Reader[Char]) extends Reader[Tokens.Token] with Reporter {
  ...
  val isKeyword    = Set("if", "else", "val", "var", "while", /*"Int", "Unit", "Boolean",*/
    "def", "true", "false", "new", "=>")
...
class BaseParser(in: Scanner) extends Parser(in) {
  ...
  def parseType: Type = in.peek match {
    //case Keyword("Int") => accept("Int");IntType
    //case Keyword("Unit") => accept("Unit");UnitType
    //case Keyword("Boolean") => accept("Boolean");BooleanType
    case Ident(x) =>
      //println(in.peek)
      if(x == "Int"){in.next();IntType}
      else if(x == "Unit"){in.next();UnitType}
      else if(x == "Boolean"){in.next();BooleanType}
      else expected("type")
...
class SyntacticSugarParser(in: Scanner) extends BaseParser(in) {
  ...
  override def parseExpression = {
    ...
    if(isNewLine(in.peek)){
      accept(';')
      Let(freshName(),UnknownType,res,super.parseExpression)
    }
    else res
  }
...
class FunctionParser(in: Scanner) extends SyntacticSugarParser(in) {
  ...
  override def parseType = in.peek match {
    case Ident(x) =>
      //println(in.peek)
      val inType = super.parseType
      //println(in.peek)
      if(Keyword("=>") == in.peek){
        accept("=>").pos
        FunType(List(("",inType)),parseType)
      }
      else inType
    case Delim('(') =>
      //println(in.peek)
      accept('(').pos
      //println(in.peek)
      val tList = parseList(("",parseType),',', tok => tok  match{
        case Ident(x) => true
        case Delim('(') => true
        case _ => false
      })
  ...
  def parseFunction: Exp = {
    ...
    accept("def")
    val (x,pos) = getName()//no check
    accept('(')
    val check = in.peek
    if(check == Delim(')')){
      accept(')')
      val ret = parseOptionalType
      accept('=')
      val rhs = parseSimpleExpression
      FunDef(x,Nil,ret,rhs).withPos(pos)
    }
    else{
      val aList = parseList(parseArg,',',tok => tok match{
        case Ident(y) => true
        case _ => false
      })
      //println(in.peek)
      ...
      accept('=')
      val rhs = parseSimpleExpression
      FunDef(x,aList,ret,rhs).withPos(pos)
    }
  }
  ...
  def parseProgram = in.peek match {
    case Keyword("def") =>
      val fList = parseList(parseFunction,';',tok => tok match{
  ...
  def parseTight = in.peek match {
    ...
    case  _ =>
      ...
      while(in.peek == Delim('(')){
        ...
        if(in.peek == Delim(')')){
          accept(')')
          res = App(res,Nil).withPos(res.pos)
        }
        else{
          val rhs = parseList(parseSimpleExpression,',',tok => true)
          accept(')')
          res = App(res,rhs).withPos(res.pos)
        }
      }
      res
  }
...
class ArrayParser(in: Scanner) extends FunctionParser(in) {
  ...
  override def parseType = in.peek match {
    case Ident("Array") => //???
      in.next()
      accept('[')
      val aType = ArrayType(super.parseType)
      accept(']')
      aType
    case _ => super.parseType
  }
  ...
  override def parseTight = {
    ...
    res match{
      case App(exp,arg) if arg.length == 1 && in.peek == Delim('=') =>
        accept('=')
        val rts = parseSimpleExpression
        Prim("block-set", List(exp, arg.head, rts))
      case _ => res
    }
  }
  ...
  override def parseSimpleExpression = in.peek match{
    case Keyword("new") =>
      ...
      val res = parseType
      accept('(')
      val rts = super.parseSimpleExpression
      accept(')')
      ArrayDec(rts,res)
    case _ => super.parseSimpleExpression
  }
