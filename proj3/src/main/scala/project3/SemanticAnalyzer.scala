...
class SemanticAnalyzer(parser: Parser) extends Reporter with BugReporter {
  ...
  def typeTerOperator(op: String)(pos: Position) = op match {
    case "block-set" => FunType(List(("", IntType), ("", IntType), ("", IntType)), UnitType)//not sure
    case _ =>
  ...
  def typeConforms(tp: Type, pt: Type)(env: TypeEnv, pos: Position): Type = (tp, pt) match {
    ...
    case (FunType(args1, rtp1), FunType(args2, rtp2)) if args1.length == args2.length =>
      val tform = typeConforms(rtp1,rtp2)(env,pos)
      val tcp = typeConform(args1,args2)(env,pos)
      FunType(tcp,tform)
  ...
  def typeInfer(exp: Exp, pt: Type)(env: TypeEnv): Exp = exp match {
    ...
    case Prim(op, args) =>
      typeOperator(op, args.length)(exp.pos) match {
        case FunType(atps, rtp) => //???
          args.length match{
            case 1 =>
              val check1 = typeCheck(args(0), atps(0)._2)(env)
              Prim(op,List(check1)).withType(rtp)
            case 2 =>
              val check1 = typeCheck(args.head, atps.head._2)(env)
              val check2 = typeCheck(args(1), atps(1)._2)(env)
              Prim(op,List(check1,check2)).withType(rtp)
            case 3 =>
              val check1 = typeCheck(args.head, atps.head._2)(env)
              val check2 = typeCheck(args(1), atps(1)._2)(env)
              val check3 = typeCheck(args(2), check1.tp)(env)
              Prim(op,List(check1,check2,check3)).withType(rtp)
          }
    ...
    case If(cond, tBranch, eBranch) =>
      ...
      // then branch.
      //???
      val tcond = typeCheck(cond,BooleanType)(env)
      val teb = typeCheck(eBranch,pt)(env)
      val ttb = typeCheck(tBranch,teb.tp)(env)
      If(tcond,teb,ttb).withType(ttb.tp)
    ...
    case VarAssign(x, rhs) =>
       ...
       */
      pt match {
        case UnitType => VarAssign(x,nrhs).withType(UnitType)//???
        case _ => VarAssign(x,nrhs).withType(nrhs.tp)//???
      }
    case While(cond, lbody, body) => //???
      val tcond = typeCheck(cond,BooleanType)(env)
      val tlb = typeCheck(lbody,UnitType)(env)
      val tb = typeCheck(body,tlb.tp)(env)
      While(tcond, tlb, tb).withType(tb.tp)
    case FunDef(fname, args, rtp, fbody) => //???
      //val tfb = typeCheck(fbody,rtp)(env)
      //checkDuplicateNames(args)
      checkDuplicateNames(args)
      val tfb = typeCheck(fbody,rtp)(env.withVals(args map{arg => (arg.name,arg.tp)}))
      FunDef(fname,args,rtp,tfb).withType(funType(args,tfb.tp))

    case LetRec(funs, body) =>
      // TODO modify to handle general case
      checkDuplicateNames(funs)

      val fList: List[(String,Type)] = funs map{
        case fun@FunDef(fname,args,rtp,_) =>
          (fname,funType(args,rtp))
      }
      //env.withVals(fList)
      val first = funs map{
        case fun@FunDef(_,args,rtp,_) =>
          typeCheck(fun, funType(args,rtp))(env.withVals(fList))
      }
      val nbody = typeCheck(body, pt)(env.withVals(fList))
      LetRec(first, nbody).withType(nbody.tp)
    case App(fun, args) =>
      // Check fun type
      val nFun: Exp = typeCheck(fun,FunType(listArgType(args.length,UnknownType),pt))(env)
      ...
      val nargs: List[Exp] = List.range(0,args.length) map {
        i => typeCheck(args(i),ftp.args(i)._2)(env)
      }
    ...
    case ArrayDec(size: Exp, etp: Type) => //???
      val sType = typeCheck(size, IntType)(env)
      ArrayDec(sType,etp).withType(ArrayType(etp))
