...
def intToBignum(x: Int):List[Int] = listMake1[Int](x);
...
def bignumPlus(b1: List[Int], b2: List[Int]): List[Int] = {
...
def bignumScale(b: List[Int], n: Int): List[Int] = {
...
def bignumTimes(b1: List[Int], b2: List[Int]): List[Int] =
  if (b1.isEmpty)
    Nil
  else
    bignumPlus(bignumScale(b2, b1.head), bignumScale(bignumTimes(b1.tail, b2), base()));

def bignumIsZero(x: List[Int]): Boolean = x.isEmpty;

def bignumFact(n: Int):List[Int] =
  if (0 == n)
    intToBignum(1)
  else
