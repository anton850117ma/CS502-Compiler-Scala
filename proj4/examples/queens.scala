...
def listRange(first: Int, to: Int): List[Int] =
  listTabulate[Int](to - first + 1, (i: Int) => first + i);

def listZipWithIndex(l: List[Int]): List[(Int, Int)] =
...
def colOk(rows: List[Int]): Boolean =
  rows.isEmpty ||
...
def onDiag(x: Int, y: Int, depth: Int): Boolean =
  x + depth == y || x - depth == y;

def diagOk(rows: List[Int]): Boolean =
  rows.isEmpty ||
...
def partialOk(rows: List[Int]): Boolean = colOk(rows) && diagOk(rows);

// not actually used in the algorithm below
def queensOk(rows: List[Int], n: Int): Boolean =
...
def printSolution(rows: List[Int]): Unit = {
