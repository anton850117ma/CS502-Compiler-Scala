...
def shuffle(l: List[Int], seed: Int): List[Int] = {
  val v = listToArray[Int](l);
  arrayShuffle[Int](v, seed);
  arrayToList[Int](v)
};

def cell(r: Int, c: Int, s: Int): Int = r * s + c;

def atE(c: Int, s: Int): Int = c + 1;

def atW(c: Int, s: Int): Int = c - 1;

def atN(c: Int, s: Int): Int = c - s;

def atS(c: Int, s: Int): Int = c + s;

// Walls
// Represented as a pair of 15-bits integers (cell indices), packed in
// a single one.

def wallMake(c1: Int, c2: Int): Int = (c1 << 15) | c2;

def wallCell1(w: Int): Int = w >> 15;

def wallCell2(w: Int): Int = w & 0x7FFF;

def isWallUp(c1: Int, c2: Int, w: List[Int]): Boolean = contains(w, wallMake(c1, c2));

// Create a maze that has walls everywhere
def completeMazeAcc(r: Int, c: Int, s: Int, acc: List[Int]): List[Int] =
  if (r < s) {
    if (c < s) {
      val rc = cell(r, c, s);
      val res1 = if (c < s - 1)
...
def completeMaze(s: Int): List[Int] =
...
def fullyDisconnectedSets(s: Int): List[List[Int]] =
...
def find(p: List[Int] => Boolean, l: List[List[Int]]): List[Int] = {
...
def connect(sets: List[List[Int]], c1: Int, c2: Int): List[List[Int]] = {
...
def printMaze(s: Int, w: List[Int]): Unit = {
  val space = ' ';
  val wall = '#';
...
def randomMaze(s: Int, seed: Int): List[Int] = {
